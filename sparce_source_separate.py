...
def record_audio(file_name, duration, fs, channels, input_device_index=None):
    p = pyaudio.PyAudio()
...
right_microphone = threading.Thread(target=record_audio, args=("./data/right_microphone.wav", 5, 44100, 1))
left_microphone = threading.Thread(target=record_audio, args=("./data/left_microphone.wav", 5, 44100, 1))

right_microphone.start()
left_microphone.start()

right_microphone.join()
left_microphone.join()
...
#音声を録音する
#record_audio("./data/left_microphone.wav", 5, 44100, 1)
#record_audio("./data/right_microphone.wav", 5, 44100, 1)

#乱数の種を初期化
np.random.seed(0)

#畳み込みに用いる音声波形
clean_wave_files=["./data/left_microphone.wav","./data/right_microphone.wav"]
